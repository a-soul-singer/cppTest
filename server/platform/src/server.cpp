...
void Server::writeJsonResponse(int clientFd, int type, const Value &body) {
...
void Server::readJsonRequest(int clientFd, int &type, Value &value) {
  Head head;
  int readSize = 0;
  readSize += read(clientFd, &head, sizeof(Head));

  int bodySize = head.length - sizeof(Head);
  char *body = new char[bodySize];
  readSize += read(clientFd, body, bodySize);
...
void Server::subThread(int fd) {
  MysqlConnection mysql;
  while (1) {
    int type = -1;
    Value value;
    readJsonRequest(fd, type, value);
    if (type == LOGIN_REQ) {
      ...
      } else if (password == res.front().front()) {
        Value responseBody;
...
int Server::CreateConnection(const Server&Server) {
  ...
  serverAddr.sin_port = htons(Server.m_port);

  // 字符串型的ip转换为int型ip格式
  // int inet_pton(int af, const char *src, void *dst);
  inet_pton(AF_INET, Server.m_ip.c_str(), &serverAddr.sin_addr.s_addr);
